...
def render(sym, setup=None, n=120):
    ...
    conn.close()
    closes = pd.Series([r[1] for r in allc],
                       index=pd.to_datetime([r[0] for r in allc]))
    ...
    path = os.path.join(OUT_DIR,
                        f"{sym}_{dt.date.today().isoformat()}.png")
    ...
    for i, (d, r) in enumerate(df.iterrows()):
        ...
        ax.add_patch(plt.Rectangle(
            ...
            width, max(1e-9, abs(r["close"] - r["open"])),
    ...
    if setup:
        ax.axhline(setup["trigger"], color="#22c55e",
                   linestyle="--", linewidth=1.2)
        ax.axhline(setup["stop"], color="#ef4444",
                   linestyle="--", linewidth=1.2)
        ax.axhline(setup["target"], color="#60a5fa",
                   linestyle=":", linewidth=1.2)
        ax.text(len(df) - 1, setup["trigger"], " trigger",
                color="#22c55e", fontsize=8, va="bottom")
        ax.text(len(df) - 1, setup["stop"], " stop",
                color="#ef4444", fontsize=8, va="top")
        ax.text(len(df) - 1, setup["target"], " target",
                color="#60a5fa", fontsize=8, va="bottom")
    step = max(1, len(df) // 6)
    ticks = list(range(0, len(df), step))
    ax.set_xticks(ticks)
    ax.set_xticklabels([df.index[i].strftime("%d-%b") for i in ticks],
                       color="#9fb0cc", fontsize=8)
    ax.tick_params(colors="#9fb0cc", labelsize=8)
    for s in ax.spines.values():
        s.set_color("#2a3550")
    ax.set_title(f"{sym} · {df.index[-1].strftime('%d %b %Y')}",
                 color="#edf3ff", fontsize=12, fontweight="bold")
    ax.legend(loc="upper left", fontsize=7, facecolor="#0b1020",
              edgecolor="#2a3550", labelcolor="#9fb0cc")
