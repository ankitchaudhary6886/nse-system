...
def compare(symbols, max_n=4):
    ...
    for sym in symbols:
        ...
        row = _row(conn, "SELECT mcap_cr, close, pe, perf1m, perf3m "
                         "FROM universe_broad WHERE symbol=?", (sym,))
        if row:
            r["mcap_cr"] = row[0]
            r["close"] = row[1]
            r["pe"] = row[2]
        ...
        # Fundamentals
        row = _row(conn, "SELECT roce, roe, pe, debt_to_equity, "
        ...
        # Latest pattern
        row = _row(conn, "SELECT pattern, direction, status, confidence "
        ...
        try:
            cached = _row(conn, "SELECT payload FROM research_cache "
                                "WHERE symbol=?", (sym,))
            if cached:
