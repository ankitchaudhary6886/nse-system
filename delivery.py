...
NSE_BHAV_URL_ALT = (
    ...
    "EQUITIES/cm/cm{date2}bhav.csv.zip"
)
...
def _parse_csv(text):
    ...
    for r in reader:
        ...
        series = (r.get("SERIES") or r.get("Series") or
        ...
        try:
            traded = int(float(
                (r.get("TTL_TRD_QNTY") or r.get("TOTAL_TRADED_QTY") or
                 r.get("TOTTRDQTY") or r.get(" TTL_TRD_QNTY") or
                 "0").strip().replace(",", "")))
            deliv = int(float(
                (r.get("DELIV_QTY") or r.get("DELIVERABLE_QTY") or
                 r.get("DELVPRCNT") or r.get(" DELIV_QTY") or
                 "0").strip().replace(",", "")))
            pct_raw = (r.get("DELIV_PER") or r.get("DLY_QT_TO_TRD_QT") or
                       r.get("DELIV_PERC") or r.get(" DELIV_PER") or
                       "").strip().replace(",", "")
            close_raw = (r.get("CLOSE_PRICE") or r.get("CLOSE") or
                         r.get(" CLOSE_PRICE") or "0").strip().replace(",", "")
...
def fetch(date_str=None):
    ...
    if text is None:
        print(f"[DELIVERY] no bhavcopy for {date_str} "
              "(holiday or not available yet)")
        return 0
    ...
    for r in rows:
        conn.execute(
            "INSERT OR REPLACE INTO delivery_daily "
            "VALUES (?,?,?,?,?,?,?)",
            (date_str, r["symbol"], r["traded_qty"],
             r["deliverable_qty"], r["delivery_pct"],
             r["close"], now))
        saved += 1
    conn.commit()
    conn.close()
...
def top(n=30, date_str=None):
    ...
    if date_str is None:
        row = conn.execute(
            "SELECT MAX(date) FROM delivery_daily").fetchone()
    ...
    for sym, pct, tq, dq, cl in rows:
        print(f"   {sym:<14} {pct:5.1f}%  traded {tq:>10,}  "
              f"deliv {dq:>10,}  close ₹{cl:.2f}")
    return [{"symbol": r[0], "delivery_pct": r[1],
             "traded_qty": r[2], "deliverable_qty": r[3],
             "close": r[4], "date": date_str} for r in rows]
...
def accumulation(n=30, min_days=5, min_avg_del=55.0):
    ...
    dates = [r[0] for r in conn.execute(
        ...
        "ORDER BY date DESC LIMIT ?", (min_days + 5,)).fetchall()]
    ...
    """.format(",".join(f"'{d}'" for d in recent_dates)),
        (min_days, min_avg_del, n)).fetchall()
    conn.close()
    out = []
    print(f"[DELIVERY] accumulation candidates "
          f"(avg delivery >= {min_avg_del}% over {min_days} sessions):")
    for sym, avg, days, tot, cl in rows:
        print(f"   {sym:<14} avg {avg:5.1f}%  "
              f"{days} sessions  total deliv {tot:>12,}  "
              f"close ₹{cl:.2f}")
        out.append({"symbol": sym, "avg_delivery_pct": round(avg, 1),
                    "sessions": days, "total_deliverable": tot,
                    "close": cl})
    return out
...
def for_symbol(symbol, limit=30):
    ...
    conn.close()
    return [{"date": r[0], "delivery_pct": r[1],
             "traded_qty": r[2], "deliverable_qty": r[3],
...
def delivery_score(symbol, conn=None, lookback=10):
    ...
    rows = conn.execute(
        ...
        "ORDER BY date DESC LIMIT ?",
        (symbol.upper(), lookback)).fetchall()
    if own:
        conn.close()
...
if __name__ == "__main__":
    ...
    if cmd == "fetch":
        ...
        for r in for_symbol(sym):
            print(f"   {r['date']}  {r['delivery_pct']:5.1f}%  "
