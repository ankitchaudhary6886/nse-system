...
def _fetch_batch(tickers, retries=3):
    ...
    for attempt in range(retries):
        try:
            ...
            last_err = e
            wait = 2 * (attempt + 1)
            log.warning(f"batch failed (attempt {attempt+1}/{retries}): "
...
def count():
    ...
    n = conn.execute(
        ...
        "WHERE data_source='tradingview'").fetchone()[0]
    n_roce = conn.execute(
        "SELECT COUNT(*) FROM fundamentals "
        "WHERE roce IS NOT NULL").fetchone()[0]
    n_roe = conn.execute(
        "SELECT COUNT(*) FROM fundamentals "
        "WHERE roe IS NOT NULL").fetchone()[0]
    n_pe = conn.execute(
        "SELECT COUNT(*) FROM fundamentals "
        "WHERE pe IS NOT NULL").fetchone()[0]
    n_de = conn.execute(
        "SELECT COUNT(*) FROM fundamentals "
        "WHERE debt_to_equity IS NOT NULL").fetchone()[0]
    n_cfo = conn.execute(
        "SELECT COUNT(*) FROM fundamentals "
        "WHERE cfo_positive IS NOT NULL").fetchone()[0]
    conn.close()
    log.info(f"fundamentals rows: {n} | "
