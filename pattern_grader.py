...
def grade_all():
    ...
    _ensure(conn)
    tags = conn.execute(
        "SELECT date, symbol, pattern, breakout_level, stop_level, "
        "direction FROM pattern_tags").fetchall()
    done = 0
    pending = 0
    skipped = 0
    for td, sym, pat, brk, stp, dirn in tags:
        exists = conn.execute(
            "SELECT 1 FROM pattern_grades WHERE tag_date=? "
            "AND symbol=? AND pattern=?",
            (td, sym, pat)).fetchone()
        if exists:
            continue
        ...
        if out == "EXPIRED" and exd is None and r == 0.0:
            ...
            continue
        conn.execute(
            "INSERT OR REPLACE INTO pattern_grades "
            "VALUES (?,?,?,?,?,?,?)",
            (td, sym, pat, out, exd, r,
...
def enabled_patterns(conn=None):
    ...
    if own:
        conn = db.get_conn()
    row = conn.execute(
        "SELECT value FROM settings WHERE key='pattern_gate'"
    ).fetchone()
    if own:
        conn.close()
    gate = {}
