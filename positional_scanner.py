...
def _quality_score(f):
    ...
    if roce is not None:
        if roce >= 25:
            ...
            score += 22; notes.append(f"ROCE {roce:.0f}")
        elif roce >= 10:
            score += 12
    ...
    if pg is not None:
        if pg >= 20:
            score += 25; notes.append(f"Profit3Y {pg:.0f}%")
        elif pg >= 15:
            score += 18
        elif pg >= 8:
            score += 10
    ...
    if sg is not None and sg >= 10:
        score += 15; notes.append(f"Sales3Y {sg:.0f}%")
    if f.get("cfo_positive") == 1:
        score += 10; notes.append("CFO+")
...
def compute(conn=None, limit=1500):
    ...
    for sym in syms:
        ...
        rows_out.append({
            ...
            "quality_score": q, "trend_score": t, "value_score": v,
    ...
    for r in rows_out:
        conn.execute(
            ...
            (r["date"], r["symbol"], r["sector"], r["last_price"],
             r["quality_score"], r["trend_score"], r["value_score"],
             r["composite"], r["tier"], r["notes"]))
    conn.commit()
    if own:
        conn.close()
...
def top(n=25, tier=None):
    ...
    _ensure(conn)
    today = conn.execute(
        "SELECT MAX(date) FROM positional_picks").fetchone()[0]
    if not today:
        conn.close()
        return []
    if tier:
    ...
    return [{"symbol": r[0], "sector": r[1], "last_price": r[2],
             "quality_score": r[3], "trend_score": r[4],
             "value_score": r[5], "composite": r[6],
             "tier": r[7], "notes": r[8]} for r in rows]


def _fmt_tg(rows):
    if not rows:
        return "📈 POSITIONAL — no candidates this week."
    lines = ["📈 POSITIONAL PICKS — long-term holds",
