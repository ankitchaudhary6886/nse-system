"""Background scheduler — EOD pipeline + swing scan + weekly meta retrain."""
...
def _drift_check():
    """Warn if recent live win-rate decays well below the model's base rate."""
    try:
        import db
        conn = db.get_conn()
        rows = conn.execute(
        ...
        if live_wr < 0.35:
            print(f"[DRIFT] ⚠️ live win-rate {live_wr:.0%} over last "
                  f"{len(rows)} graded — consider retraining / review")
        else:
...
def _retrain_job():
    ...
    try:
        import meta_model
        meta_model.train()
        meta_model._MODEL = None   # force reload of fresh model
...
def start():
    ...
    _scheduler.start()
    print("[SCHEDULER] started — daily@15:45, swing@16:15, retrain@Sat 09:00 IST")
