...
function setView(name) {
  ...
  $("viewTitle").textContent =
    name === "overview" ? "Overview" : name === "swing" ? "Swing Desk" : "Radar";
...
async function loadRegime() {
  ...
  try {
    ...
    if (r.stance === "BULLISH") {
      ...
      box.innerHTML = `🟢 <strong>BULLISH / AGGRESSIVE</strong> — ${r.symbol} close ${r.index_close} > EMA10 ${r.ema10}. New long setups allowed.`;
    } else if (r.stance === "DEFENSIVE") {
      box.classList.add("defensive");
      box.innerHTML = `🛑 <strong>DEFENSIVE / CASH</strong> — ${r.symbol} close ${r.index_close} < EMA10 ${r.ema10}. No new long entries.`;
    } else box.innerHTML = `⚠️ Regime unavailable: ${r.error || ""}`;
  } catch (e) {
    box.classList.add("defensive");
...
function trancheLadder(setup) {
  ...
  return `
    ...
    <div class="level"><span>Stage 2 · +2R → SL to breakeven</span><strong>${l2.toFixed(2)}</strong></div>
    <div class="level"><span>Stage 3 · +4R → exit 1/3rd</span><strong>${l4.toFixed(2)}</strong></div>
...
async function loadSwing() {
  ...
  $("mOpen").textContent = `${score.OPEN || 0} / ${score.PENDING || 0}`;

  for (const s of signals) {
    const tr = document.createElement("tr");
    tr.innerHTML = `
      <td>${s.date}</td><td><strong>${s.symbol}</strong></td>
      ...
      <td>${s.ema_zone || "—"}</td><td>${outcomeBadge(s.outcome)}</td>`;
...
async function loadRadar() {
  ...
  m.innerHTML = ""; v.innerHTML = ""; t.innerHTML = "";
  (g["Momentum"] || []).slice(0, 18).forEach(x => m.appendChild(createRadarCard(x)));
  (g["Volume Spike"] || []).slice(0, 18).forEach(x => v.appendChild(createRadarCard(x)));
  (g["Turnaround"] || []).slice(0, 18).forEach(x => t.appendChild(createRadarCard(x)));
}
...
async function loadSymbol(symbol) {
  ...
  $("chartSubtitle").textContent =
    `${summary.sector || "Unknown"} · ₹${fmt(summary.mcap_cr)} cr · Fund ${fmt(summary.fund_score)}`;
...
async function runSwingScan() {
  ...
  try {
    ...
    setTimeout(async () => { await loadSwing(); $("runSwingBtn").textContent = "Run Swing Scan"; }, 4000);
  } catch (e) {
