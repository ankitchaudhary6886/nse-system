...
SAMPLE = [
    ...
    "SUNPHARMA", "MARUTI", "ASIANPAINT", "BAJFINANCE", "TITAN",
    "DIXON", "POLYCAB", "KEI", "ASTRAL", "PIIND",
    "PERSISTENT", "KPITTECH", "TATAELXSI", "SYNGENE", "ALKEM",
    "APARINDS", "GRANULES", "JBCHEPHARM", "NAVINFLUOR", "TATAINVEST",
]
...
CANDIDATE_COLUMNS = [
    ...
    # ---- Valuation ----
    "price_earnings_ttm", "price_earnings_forward",
    "price_earnings_fq", "price_earnings_fy",
    "price_to_book", "price_to_book_fq", "price_to_book_fy",
    "price_to_sales_ttm", "price_to_sales_fq",
    "price_to_cash_fq", "price_to_cash_fy",
    "enterprise_value_ebitda_ttm",
    "enterprise_value_ebitda_fq",
    "ev_to_revenue_fq", "ev_to_revenue_ttm",
    ...
    "debt_to_equity",
    "debt_to_assets_fq",
    "total_debt_fq", "total_debt_fy",
    "total_debt_to_ebitda_fq",
    "net_debt_fq", "net_debt_fy",
    "cash_and_equivalents_fq", "cash_and_equivalents_fy",

    # ---- Growth ----
    "revenue_growth_fy", "revenue_growth_yoy",
    "revenue_growth_3y", "revenue_growth_5y",
    "revenue_growth_qoq",
    "net_income_growth_fy", "net_income_growth_yoy",
    "net_income_growth_3y", "net_income_growth_5y",
    "net_income_growth_qoq",
    "eps_growth_fy", "eps_growth_yoy",
    "eps_growth_3y", "eps_growth_5y",

    # ---- Cash flow ----
    "free_cash_flow_fq", "free_cash_flow_fy",
    "operating_cash_flow_fq", "operating_cash_flow_fy",
    "cash_from_operations_fq",
    "cash_from_operations_fy",
    "capital_expenditures_fq", "capital_expenditures_fy",
...
def main():
    ...
    print(f"[PROBE] {len(tickers)} sample symbols, "
    ...
    stats = {col: {"non_null": 0, "non_zero": 0, "sample": None}
    ...
    for col in CANDIDATE_COLUMNS:
        s = stats[col]
        marker = "✓" if s["non_zero"] > 0 else \
                 ("~" if s["non_null"] > 0 else "✗")
        sample_str = str(s["sample"])[:30] if s["sample"] is not None else ""
        print(f"{col:<40} {s['non_null']:<10} {s['non_zero']:<10} "
              f"{marker}  {sample_str}")

    print()
    print("Legend: ✓ returns real values  ~ accepted but null "
          "✗ column name rejected")
    print()

    # Summary lists
    working = [c for c in CANDIDATE_COLUMNS
               if stats[c]["non_zero"] > 0]
