...
def _quality_score(row):
    ...
    if roce is not None:
        if roce >= 15:
            score += 30; notes.append(f"ROCE {roce:.0f}")
        elif roce >= 10:
            score += 15; notes.append(f"ROCE {roce:.0f}")
    de = row.get("debt_to_equity")
    if de is not None:
        if de <= 1.0:
            score += 20; notes.append(f"D/E {de:.2f}")
        elif de <= 2.0:
            score += 10; notes.append(f"D/E {de:.2f}")
    prom = row.get("promoter_holding")
    if prom is not None:
        if prom >= 40:
            score += 20; notes.append(f"Prom {prom:.0f}%")
        elif prom >= 25:
            score += 10; notes.append(f"Prom {prom:.0f}%")
    if row.get("cfo_positive") == 1:
        score += 15; notes.append("CFO+")
    pg = row.get("profit_growth_3y")
    if pg is not None:
        if pg >= 15:
            score += 15; notes.append(f"Profit3Y {pg:.0f}%")
        elif pg >= 8:
            score += 8; notes.append(f"Profit3Y {pg:.0f}%")
    return score, notes
...
def compute(conn=None, limit=1500):
    ...
    fund_cols = [r[1] for r in conn.execute(
    ...
    for sym in syms:
        ...
        rows_out.append({
            ...
            "pe": pe, "sector_pe": sec_med,
            "quality_score": q_score, "value_score": v_score,
    ...
    for r in rows_out:
        conn.execute(
            ...
            "(?,?,?,?,?,?,?,?,?,?,?,?)",
            (r["date"], r["symbol"], r["sector"], r["last_price"],
             r["below_52w"], r["pe"], r["sector_pe"],
             r["quality_score"], r["value_score"], r["composite"],
             r["tier"], r["notes"]))
    conn.commit()
    if own:
        conn.close()
...
def top(n=25, tier=None):
    ...
    _ensure(conn)
    today = conn.execute(
        "SELECT MAX(date) FROM value_radar").fetchone()[0]
    if not today:
        conn.close()
        return []
    if tier:
        rows = conn.execute(
            ...
            "ORDER BY composite DESC LIMIT ?", (today, tier, n)).fetchall()
    else:
        rows = conn.execute(
            "SELECT symbol, sector, last_price, below_52w, pe, sector_pe, "
            "quality_score, value_score, composite, tier, notes "
            "FROM value_radar WHERE date=? "
            "ORDER BY composite DESC LIMIT ?", (today, n)).fetchall()
    conn.close()
    return [{"symbol": r[0], "sector": r[1], "last_price": r[2],
             "below_52w": r[3], "pe": r[4], "sector_pe": r[5],
             "quality_score": r[6], "value_score": r[7],
             "composite": r[8], "tier": r[9], "notes": r[10]}
            for r in rows]
...
def _fmt_tg(rows):
    ...
    lines = ["📉 VALUE RADAR — quality names in downturns",
             f"{len(rows)} candidates (top 8 shown)"]
    for r in rows[:8]:
        lines.append(
            f"[{r['tier']}] {r['symbol']}  "
            f"₹{r['last_price']}  "
